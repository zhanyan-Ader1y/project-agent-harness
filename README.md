...
**部件齐了，闭环还差最后一步。** 5 个脚本 + 4 条 hook + 2 个 skill，2,532 行，**8 份 eval 共 545 项断言**，CI 双 OS matrix 全绿。
...
| 意图确立 | `intent-lock` skill + 提交时的偏离提醒（`hooks/intent-drift.js`） | ✅ 已落地。**是提醒不是拦截**——解决"忘了对照"，不解决"不许偏离" |
| review 增强 | — | ❌ **现在最空的一条**，整条延后；也是延后表里成本最低的一个 |
